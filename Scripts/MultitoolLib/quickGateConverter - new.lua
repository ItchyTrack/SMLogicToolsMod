...
function Converter.sv_convertBody(self, data)
    ...
    local jsontable = sm.creation.exportToTable(body)
    --print(jsontable.bodies)
    --TODO code a more reliable version without bitmanupulation
    local colorlookup = {}
    local partdata = {}
    if wantedType == "QuickLogic" then
        for i = 1, #jsontable.bodies do
            for j = 1, #jsontable.bodies[i].childs do
                if jsontable.bodies[i].childs[j].shapeId == "9f0f56e8-2c31-4d83-996c-d00a9b296c3f" then
                    local color = jsontable.bodies[i].childs[j].color
                    local mode = jsontable.bodies[i].childs[j].mode
                end
            end
        end
        --[[for i = 1, #jsontable.bodies do
