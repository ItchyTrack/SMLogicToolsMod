...
QuickTimer.partFunction = function(ids, datalist, parentcount, counttable, newstates, newmemberids, oldstates)
    for index=1, #ids do
        local id = ids[index]
        local state = counttable[id] == 1 --because it has only one parent
        local data = datalist[id]

        if data.time == 0 then
            newstates[id] = state
        else
            if data.delayTable[data.pointer] ~= state then
                data.forceUpdateCounter = data.time
            end
            ...
            data.delayTable[data.pointer] = state
        end

        if data.forceUpdateCounter ~= 0 then
            data.forceUpdateCounter = data.forceUpdateCounter - 1
...
function QuickTimer.sv_saveTime(self, data)
    ...
    for i = 1, self.data.time do
        self.data.delayTable[i] = false
    end
    self.data.pointer = 1
...
function QuickTimer.server_onCreate(self )
    ...
    self.data.pointer = 1
    self.data.forceUpdateCounter = 0
    signin(self, "QuickTimer", QuickTimer.partFunction, true)
    self.network:setClientData({ticks = self.ticks, seconds = self.seconds})
    self.storage:save({ticks = self.ticks, seconds = self.seconds})
end
